...
void main() {
  group('BreedSearchField', () {
    ...
    testWidgets('renders without error', (tester) async {
      await tester.pumpWidget(testApp(const BreedSearchField(label: 'Breed')));
    ...
    testWidgets('displays label text', (tester) async {
      await tester.pumpWidget(
        testApp(const BreedSearchField(label: 'Dog Breed')),
      );
    ...
    testWidgets('shows initial value when provided', (tester) async {
      await tester.pumpWidget(
        testApp(const BreedSearchField(label: 'Breed', initialValue: 'Beagle')),
      );
      final field = tester.widget<TextField>(find.byType(TextField));
    ...
    testWidgets('shows a 2px primary focus ring only while focused', (
      ...
      expect((idleDecoration.border as Border).top.color, Colors.transparent);
    ...
    testWidgets(
      'selecting a breed calls onChanged, fills the field, and closes the list',
      (tester) async {
        String? selected;
        await tester.pumpWidget(
          testApp(
            BreedSearchField(label: 'Breed', onChanged: (v) => selected = v),
          ),
        );
    ...
    testWidgets('trigger container uses the DS Input surface fill', (
      tester,
    ) async {
      await tester.pumpWidget(testApp(const BreedSearchField(label: 'Breed')));
    ...
    testWidgets('selected breed highlight uses accentPurpleTile', (
      tester,
    ) async {
      await tester.pumpWidget(
        testApp(const BreedSearchField(label: 'Breed', initialValue: 'Akita')),
      );
    ...
    testWidgets(
      'hovering an unselected option highlights it with accentPurpleTile',
      (tester) async {
        await tester.pumpWidget(
          testApp(const BreedSearchField(label: 'Breed')),
        );
