...
void main() {
  group('ResponsiveLayout', () {
    ...
    testWidgets('falls back to mobile when tablet not provided', (
      ...
      await tester.pumpWidget(
        buildTestWidget(width: 1000, mobile: (_) => const Text('mobile')),
      );
      expect(find.text('mobile'), findsOneWidget);
    });
  });
  ...
  group('ResponsiveContext extension — isTablet consistency with ResponsiveLayout', () {
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          ...
          isFalse,
          reason:
              'screenWidth 800 >= kMobileBreakpoint 600, so isMobile should be false',
        );
        expect(
          capturedContext!.isTablet,
          isFalse,
          reason:
              'screenWidth 800 < kTabletBreakpoint 960, so isTablet should be false — '
    ...
    testWidgets('at 1000px: isTablet == true (matches ResponsiveLayout tablet)', (
      ...
      expect(
        ...
        isTrue,
        reason:
            'screenWidth 1000 is in [kTabletBreakpoint 960, kDesktopBreakpoint 1200)',
      );
