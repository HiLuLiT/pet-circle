...
| Widget | File | Used In |
|--------|------|---------|
| `BreedSearchField` | `lib/widgets/breed_search_field.dart` | Onboarding Step 1 (the Pet Detail edit sheet was removed with that screen -- BUG-056) |
...
| Functions CI | `.github/workflows/ci.yml` runs Flutter jobs only, so TypeScript breakage in `functions/` ships unnoticed. The scripts exist (`npm --prefix functions run typecheck`, `npm --prefix functions test` — the latter builds then runs `node --test`, no extra dependencies); CI just needs a Node 20 job that calls them |
...
| Dead settings | `emergencyAlerts` persists to Firestore but drives no behaviour. (The global medication morning/evening times were also dead; they have been removed in favour of per-medication `reminderTimes` set in the Add/Edit Medication sheet) |
...
|----|---------|---------|----------|
| FB-001 | Pet photo upload | Replace photo URL text field with image picker + Firebase Storage upload. Affects onboarding step 1 and pet card display (the pet detail edit sheet was removed -- BUG-056). Uses `image_picker` (already in pubspec). Requires adding `firebase_storage` dependency and a `StorageService` for upload/download URLs. | Medium |
...
| FB-004 | Invite abuse prevention — **server-side rate limiting** | **Raised to High by the Phase 2 security review.** The daily cap (`InvitationService.maxInvitesPerDay = 20`) is enforced *only on the client*, in `validateInvitation` call sites. Nothing in `firestore.rules` or in the trigger enforces it, and the Firebase Web API key is public, so writes made straight to the Firestore SDK/REST endpoint bypass it entirely. Meanwhile `canCreateInvitation()` requires only pet ownership, `canCreatePet()` is open to any authenticated user, and signup is self-service via the public `sendOTP`/`verifyOTP` callables — so any signed-up user can create a pet and then emit unbounded email, with attacker-authored subject and body, to arbitrary recipients from the project's verified Resend sender. Consequences: spam/phishing relay, Resend quota exhaustion, sending-domain reputation damage. BUG-041 bounded the *content* (flatten + truncate + escape, plus a 200-char server-side ceiling in the rules) but deliberately did not address *volume*. Fix: either move creation behind an authenticated `onCall` that owns the counter and the write (`allow create: if false` on `/invitations`), or keep the client write and require a UTC-day-keyed `users/{uid}/inviteQuota` counter to be incremented in the same batch with a `count < N` guard. Also still open from the original entry: email format validation, duplicate-invite prevention, and a care-circle size cap. | High |
